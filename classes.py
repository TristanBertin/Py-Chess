...
def get_deplacement_list(x_i, y_i, x_f, y_f):
    L = []
    x = x_i
    y = y_i
...
class Piece:
    ...
    def get_final_deplacements(self, own_list, adv_list):
        ...
        for pos_dep in list_of_possible_deplacements:
            ...
            for k, adv_piece in enumerate(adv_list):  #we tcheck after removing adverse piece if chess
...
class Pion(Piece):
    ...
    def get_possible_deplacements_without_chess(self,own_list,adv_list):
        ...
        for i in range(8):
            for j in range(8):
                (x_fin, y_fin) = (i,j)
                if ([x_fin, y_fin] not in own_pieces_pos.reshape(-1, 2).tolist()):

                    if self.player == 0:

                        if (((self.y_pos ==6 and y_fin==4) and (x_fin==self.x_pos)) and ([x_fin, y_fin] not in adv_pieces_pos.reshape(-1, 2).tolist())) or \
                            (y_fin == self.y_pos - 1 and x_fin == self.x_pos) and ([x_fin, y_fin] not in adv_pieces_pos.reshape(-1, 2).tolist()) or \
                            ((self.y_pos-y_fin)==1 and abs((self.x_pos-x_fin) * (y_fin - self.y_pos)) == 1) and ([x_fin, y_fin] in adv_pieces_pos.reshape(-1, 2).tolist()):

                            deplacements_list = get_deplacement_list(self.x_pos, self.y_pos, x_fin, y_fin)
                            assert_mov = True
                            for intermediary_case in deplacements_list:
                                if (intermediary_case in own_pieces_pos.reshape(-1, 2).tolist()) or (intermediary_case in adv_pieces_pos.reshape(-1, 2).tolist()):
                                    assert_mov = False
                    ...
                    if self.player == 1:
                        ...
                        if (((self.y_pos ==1 and y_fin==3) and (x_fin==self.x_pos))and ([x_fin, y_fin] not in adv_pieces_pos.reshape(-1, 2).tolist())) or \
                                (y_fin == self.y_pos + 1 and x_fin == self.x_pos and ([x_fin, y_fin] not in adv_pieces_pos.reshape(-1, 2).tolist())) or \
                                (((self.y_pos - y_fin) == -1 and abs((self.x_pos - x_fin) * (y_fin - self.y_pos)) == 1) and ([x_fin, y_fin] in adv_pieces_pos.reshape(-1, 2).tolist())):
...
class Fou(Piece):
    ...
    def get_possible_deplacements_without_chess(self, own_list, adv_list):
        ...
        for i in range(8):
            for j in range(8):
                (x_fin, y_fin) = (i, j)
                if (([x_fin, y_fin] not in own_pieces_pos.reshape(-1, 2).tolist()) and (
                        abs(y_fin - self.y_pos) == abs(x_fin - self.x_pos))):
                    deplacements_list = get_deplacement_list(self.x_pos, self.y_pos, x_fin, y_fin)
                    assert_mov = True
                    for intermediary_case in deplacements_list:
                        if (intermediary_case in own_pieces_pos.reshape(-1, 2).tolist()) or (
                                intermediary_case in adv_pieces_pos.reshape(-1, 2).tolist()):
                            assert_mov = False
                    if assert_mov:
                        L.append([i, j])
            ...
            for j in range(8):
                ...
                if ([x_fin, y_fin] not in own_pieces_pos.reshape(-1, 2).tolist()):
                    if abs(y_fin - self.y_pos) in (0, 1) and abs(x_fin - self.x_pos) in (0, 1):
                        L.append([i, j]) #assert mov ok for king
